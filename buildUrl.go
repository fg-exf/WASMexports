...
func message(name string) {
	log(fmt.Sprint("Building Request URI in WASM:  ", formatting(name)))
}
...
// formatting gets a formatted message for the name.
func formatting(name string) string {
	return fmt.Sprint("https://", name)
}

// _message is a WebAssembly export that accepts a string pointer (linear memory
// offset) and calls message.
//
//go:wasmexport message
func _message(ptr, size uint32) {
	name := ptrToString(ptr, size)
	message(name)
}

// _formatting is a WebAssembly export that accepts a string pointer (linear memory
...
// WebAssembly 1.0.
//
//go:wasmexport formatting
func _formatting(ptr, size uint32) (ptrSize uint64) {
	name := ptrToString(ptr, size)
	f := formatting(name)
	ptr, size = stringToLeakedPtr(f)
